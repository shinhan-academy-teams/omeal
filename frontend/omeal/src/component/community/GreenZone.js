  ...
  const searchSubmit = () => {
    ...
    if (selectedOption) {
      if (searchCategory === "") {
        ...
        axios({
          method: "get",
          url: `/board/녹색지대/${selectedOption}/10`,
        })
          .then((res) => {
            setPostList(res.data.map((data) => data));
          })
        ...
        axios({
          method: "get",
          url: "/board/녹색지대/" + selectedOption + "/" + searchCategory,
          params: {
            topic: search,
          },
        })
