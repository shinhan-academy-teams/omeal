...
function RiceSoupMinistry(props) {
  ...
  const searchSubmit = () => {
    ...
    if (selectedOption) {
      if (searchCategory === "") {
        ...
        axios({
          method: "get",
          url: `/board/국밥부/${selectedOption}/10`,
        })
          .then((res) => {
            setPostList(res.data.map((data) => data));
          })
        ...
        axios({
          method: "get",
          url: "/board/국밥부/" + selectedOption + "/" + searchCategory,
          params: {
            topic: search,
          },
        })
...
export default RiceSoupMinistry;
